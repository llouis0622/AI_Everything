...
- 사이클 형성 가능, 이웃한 정점끼리의 상하 관계 없음
- 연결 그래프 : 그래프 상에 있는 임의의 두 정점 사이의 경로가 존재하는 그래프
- 비연결 그래프 : 어떤 정점 사이에는 경로가 존재하지 않을 수 있는 그래프
- 방향 그래프 : 간선에 방향이 있는 그래프
- 무방향 그래프 : 간선에 방향이 없는 그래프
- 가중치 그래프 : 간선에 가중치(비용)가 부여된 그래프
- 서브그래프 : 특정 그래프의 정점과 간선의 일부분으로 이루어진 그래프

### 인접 행렬 기반 그래프 표현

- N x N 크기의 행렬로 그래프를 표현하는 방법
- N 정점의 개수, <행, 열> 값 <출발 정점, 도착 정점>
- 두 정점 연결 시 1, 미연결 시 0
- 무방향 그래프인 경우 행렬의 대각선 요소를 기준으로 연결 관계가 대칭을 이룸
- 가중치 그래프 : <출발 정점, 도착 정점>을 연결하는 가중치로 표현

### 인접 리스트 기반 그래프 표현

- 그래프의 특정 정점과 연결된 정점들을 연결 리스트로 표현하는 방법
- 특정 정점에서 나가는 간선에 연결된 정점들을 연결 리스트의 노드로 삼음
- 가중치 그래프 : 하나의 노드를 표현하는 정보에 가중치 정보까지 포함
...
- 인접한 모든 정점들을 방문하고, 방문한 정점들과 연결된 모든 정점들을 방문하기를 반복하는 탐색 방법
- 배열 : 특정 정점의 방문 여부 확인
- 큐 : 연결된 정점들을 저장
...
- 다익스트라 작동 과정
    ...
    - 더 이상 방문할 정점이 없을 때까지 과정 반복 후 종료